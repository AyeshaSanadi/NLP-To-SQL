...
## Instructions
- First explain your reasoning step by step for generating the SQL.
- Then generate a valid SQL query.
...
sql: Step 1: We need artifacts that have a property `git_repo`.
Step 2: Filter artifacts where `git_repo` contains `mode_classifier`.
Step 3: Join artifact with artifactproperty on artifact_id.
Step 4: Select all artifact columns.
SQL:
SELECT a.*
FROM artifact a
...
sql: Step 1: Identify executions with property `Pipeline_Type`.
Step 2: Filter executions where `Pipeline_Type` equals `Training-percent_3`.
Step 3: Join execution with executionproperty on execution_id.
Step 4: Select all execution columns.
SQL:
SELECT e.*
FROM execution e
...
sql: Step 1: Look for artifacts whose `name` contains `cnn_lh_predictor`.
Step 2: Filter artifact table by name using LIKE.
Step 3: Select id and name columns.
SQL:
SELECT id, name
FROM artifact
...
sql: Step 1: Identify artifacts where properties `model_name`, `model_framework`, `model_type` are null.
Step 2: Use NOT EXISTS subqueries to check each property.
Step 3: Select all artifact columns.
SQL:
SELECT a.*
FROM artifact a
WHERE NOT EXISTS (
...
sql: Step 1: Look for artifacts with property `model_name`.
Step 2: Filter where `model_name` equals `TCNN1d`.
Step 3: Join artifact and artifactproperty tables.
Step 4: Select artifact id, name, and property value.
SQL:
...
sql: Step 1: Identify executions with property `train_percent`.
Step 2: Filter executions where `train_percent` equals 0.9.
Step 3: Join execution and executionproperty tables.
Step 4: Select execution id, name, and train_percent value.
SQL:
...
sql: Step 1: Identify artifacts of type `Dataslice`.
Step 2: Filter by artifact name exactly matching the given dataslice path.
Step 3: Join artifact with type table on type_id.
Step 4: Select all artifact columns.
SQL:
SELECT a.*
FROM artifact a
...
sql: Step 1: Convert `create_time_since_epoch` to date format.
Step 2: Filter artifacts where the converted date equals '2025-09-09'.
Step 3: Select all artifact columns.
SQL:
SELECT a.*
FROM artifact a
...
sql: Step 1: Look for executions with property `output_classes`.
Step 2: Filter where `output_classes` matches the list ['BBQH', 'QH', 'WPQH'].
Step 3: Join execution and executionproperty tables.
Step 4: Select all execution columns.
SQL:
SELECT e.*
...
sql: Step 1: Look for executions with property `training_files`.
Step 2: Count number of files by counting commas in `string_value` and adding 1.
Step 3: Filter executions where count < 40.
Step 4: Join execution and executionproperty tables.
Step 5: Select all execution columns.
SQL:
SELECT e.*
FROM execution e
JOIN executionproperty ep ON e.id = ep.execution_id
WHERE ep.name = 'training_files'
  AND (LENGTH(ep.string_value) - LENGTH(REPLACE(ep.string_value, ',', '')) + 1) < 40;
